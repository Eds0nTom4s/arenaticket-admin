    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="card bg-gradient-to-br from-blue-500 to-blue-600 text-white">
        <div class="flex items-start justify-between">
          <div>
            <div class="text-sm text-blue-100 mb-1">Total de Eventos</div>
            <div class="text-3xl font-bold">{{ totalEventos }}</div>
            <div class="text-xs text-blue-100 mt-2">{{ eventosAtivos }} ativos</div>
          </div>
      ...
      <div class="card bg-gradient-to-br from-green-500 to-green-600 text-white">
        <div class="flex items-start justify-between">
          <div>
            <div class="text-sm text-green-100 mb-1">Bilhetes Vendidos</div>
            <div class="text-3xl font-bold">{{ pedidosPagos }}</div>
            <div class="text-xs text-green-100 mt-2">Pedidos confirmados</div>
          </div>
          <div class="p-3 bg-white/20 rounded-lg">
            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div class="card bg-gradient-to-br from-purple-500 to-purple-600 text-white">
        <div class="flex items-start justify-between">
          <div>
            <div class="text-sm text-purple-100 mb-1">Receita Total</div>
            <div class="text-2xl font-bold">{{ formatCurrency(receitaTotal) }}</div>
            <div class="text-xs text-purple-100 mt-2">Em vendas</div>
          </div>
          <div class="p-3 bg-white/20 rounded-lg">
            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div class="card bg-gradient-to-br from-orange-500 to-orange-600 text-white">
        <div class="flex items-start justify-between">
          <div>
            <div class="text-sm text-orange-100 mb-1">Próximos Eventos</div>
            <div class="text-3xl font-bold">{{ proximosEventos.length }}</div>
            <div class="text-xs text-orange-100 mt-2">Aguardando realização</div>
          </div>
          <div class="p-3 bg-white/20 rounded-lg">
            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="card">
        <div class="flex items-center justify-between mb-4">
          ...
          <router-link to="/eventos" class="text-sm text-[var(--color-cyan)] hover:underline">
            Ver todos
          </router-link>
        </div>
        ...
        <div v-else class="space-y-3">
          ...
          >
            <div class="flex-1">
              <div class="font-medium text-sm">{{ evento.titulo }}</div>
            ...
            <router-link 
              ...
              class="text-xs px-3 py-1.5 bg-[var(--color-cyan)] text-white rounded-lg hover:opacity-90 transition-opacity"
            >
              Lotes
            </router-link>
          </div>
      ...
      <div class="card">
        <div class="flex items-center justify-between mb-4">
          ...
          <router-link to="/pedidos" class="text-sm text-[var(--color-cyan)] hover:underline">
            Ver todos
          </router-link>
        </div>
        ...
        <div v-else class="space-y-3">
          ...
          >
            <div class="flex-1">
              <div class="font-medium text-sm">{{ pedido.compradorNome }}</div>
              <div class="text-xs text-[var(--color-text-secondary)] mt-1">
                {{ formatDate(pedido.createdAt) }}
              </div>
            </div>
            <div class="text-right">
              <div class="font-semibold text-sm">{{ formatCurrency(pedido.valorTotal) }}</div>
